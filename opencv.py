...
ranges = np.zeros((muser, 2, 3), dtype=np.uint8)

colors_by_command_key = {'r': RED, 'g': GREEN, 'b': BLUE, 'y': YELLOW, 'o': ORANGE, 'k': BLACK, 'p':BROWN}
selected_colors = [RED, GREEN, BLUE, YELLOW, ORANGE, BLACK, BROWN]
...
while True:
    ...
    resized_unflipped = cv.resize(large_frame, (0, 0), fx=0.75, fy=0.75)
    ...
    def mask_for_user(i):
        return cv.inRange(resized_flipped_hsv, ranges[i, 0], ranges[i, 1])
        ...
        for player_index, mask in players_and_masks:
            ...
            if key_points:
                ...
                if pt[0] < 100 and pt[1] < 100:
                    accumulator_frame[:, :, :] = 0
                cv.circle(accumulator_frame, (round(pt[0]), round(pt[1])), 7, selected_colors[player_index], -1)
